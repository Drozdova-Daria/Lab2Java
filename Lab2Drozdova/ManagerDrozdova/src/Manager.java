...
public class Manager {
    ...
    private RC pipelineBuild(String[] workersClass, String[] workersConfigs) {
        ...
        error = setWorkers(reader, executors, writer);
        if (error != RC.CODE_SUCCESS) {
            return error;
        }
        error = reader.execute(data);
        return error;
    }

    private RC setWorkers (IReader reader, IExecutor[] executors, IWriter writer) {
        RC error;
        ...
        if(executors.length == 1) {
            error = executors[0].setConsumer(writer);
        } else {
